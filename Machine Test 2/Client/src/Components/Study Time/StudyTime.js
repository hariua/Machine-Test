...
export default function StudyTime() {
    ...
    const [studyUsage, setStudyUsage] = useState('0')
    useEffect(() => {
        axios.get('https://api.mocklets.com/mock68182/screentime').then((response) => {

            let devUsage = response.data[0].deviceUsage.studyTime
            console.log(devUsage, "gl");
            ...
            let studyTime = parseInt(response.data[0].chartData.studyTime.total)
            if (studyTime > 60) {
                let hrs = studyTime / 60
                let hr = parseInt(hrs)
                let min = studyTime - (hr * 60)
                studyTime = hr + "H : " + min
                setStudyUsage(studyTime)
            } else {
                setStudyUsage(studyTime)
            }
            const data = [
            ...
            <Container maxWidth="md">
                <Grid container className="containerBox" direction="row" justify="space-evenly">
                    <Grid alignItems="center" item xs="12" md="6" sm="6" lg="6">
                        {graphData.length > 0 ?
                            <div>
                                <Typography align="center" variant="h6" className="allScreenHead">Study Time</Typography>
                                ...
                                <Grid container justify="space-evenly" direction="row" className="chartText">
                                    <Typography align="center" variant="subtitle2">Study Time:<br></br>{studyUsage + " Mins"}</Typography>
